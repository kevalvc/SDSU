...
% ObadjImgect classification is an important task in many computer vision
...
% to classify nearby obadjImgects as pedestrians or vehicles. Regardless of the
% type of obadjImgect being classified, the basic procedure for creating an
% obadjImgect classifier is:
% * Acquire a labeled data set with images of the desired obadjImgect.
...
% The flowerImageSet is an array of 3 imageSet obadjImgects: one for each flower.
...
for flower = 1:numel(flowerImageSet)
    ...
    for i = 1:numImages
        ...
        x1=extractHOGFeatures(img);
        x1=x1(x1 > 0.1);
        features(1,:) = x1(1:128);   

        %Using histogram of color
        adjustedImg = imadjust(img, [0.0 1.0], [0.0 0.5]);
        histImg = imhist(adjustedImg);
        histImg = histImg(1:128);
        histImg = histImg';
        features(i, :) = histImg;

        %Using SIFT(SURF) features
        corner = detectSURFFeatures(img);
        corSt = corner.selectStrongest(128);
        [feature, vp] = extractFeatures(img,corSt);
        f1 = sum(feature, 2).';
        [m, n] = size(f1);
        for j = 1:n
            features(i,j) = f1(j);
...
% Code for failure images

imgSet = numel(arrTestID);
 for i=1:imgSet
     if(testLabels(i,7) ~= predictedLabels(1,7))
        img = rgb2gray(read(flowerImageSet(3), arrTestID(i)));
        figure;
        imshow(img);
        title(strcat('TestLabel: ', testLabels(i,7) , ' PredictedLabel: ',predictedLabels(i,7)));
        break 
     end
end
%%
...
% obadjImgect classifier using the feautre descriptors and the |fitcecoc| function from the
